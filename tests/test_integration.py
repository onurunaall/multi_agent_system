...
async def test_router_to_music_agent_flow(mocker):
    ...
    correctly mocking all external calls with precision.
    """
    from workflow import multi_agent_final_graph

    # 1. Mock async LLM invocations for the router and the agent
    mock_llm_ainvoke = mocker.patch(
        "langchain_openai.ChatOpenAI.ainvoke",
        new_callable=AsyncMock,
        side_effect=[AIMessage(content="",
                               tool_calls=[{"name": "RouteQuery",
                                            "args": {"destination": "music"},
                                            "id": "test_call_router"}]),
                    AIMessage(content="",
                              tool_calls=[{"name": "search_for_music",
                                           "args": {"query": "classic rock"},
                                           "id": "call_xyz"}]),
                    AIMessage(content="Here is a classic rock song I found.")])

    mock_retriever_invoke = mocker.patch("agents.music_agent.vector_retriever.similarity_search",
                                         return_value=[Document(page_content="Track: Stairway to Heaven by Led Zeppelin")])

    config = {"configurable": {"thread_id": "integration-test-final-final-run"}}
    ...
    assert mock_llm_ainvoke.call_count == 3
    mock_retriever_invoke.assert_called_once_with("classic rock", k=5)
